...
export default function App() {
  ...
  const activeProjectStories = useMemo(() => activeProject ? stories.filter(s => s.projectId === activeProject.id) : [], [activeProject, stories]);
  ...
  const handleProjectClick = (projectId) => {
    ...
    setProjects(projects.map(p => p.id === projectId ? { ...p, isExpanded: true } : p));
  };

  const handleStoryClick = (storyId, projectId) => {
    setActiveStoryId(storyId);
    setActiveProjectId(projectId);
  };

  const toggleProjectExpansion = (e, projectId) => {
    e.stopPropagation();
    setProjects(projects.map(p => p.id === projectId ? { ...p, isExpanded: !p.isExpanded } : p));
  };

  const handleRenameProject = (e, projectId, currentName) => {
    e.stopPropagation();
  ...
  const confirmRenameProject = () => {
    if (renameProjectName && renameProjectName.trim() !== "") {
      setProjects(projects.map(p => p.id === projectToRenameId ? { ...p, name: renameProjectName } : p));
  ...
  const confirmDeleteProject = () => {
    ...
    const newStories = stories.filter(s => s.projectId !== projectToDelete);
  ...
  const handleAddStory = (projectId) => {
    const newStory = {
      id: Date.now(),
      projectId: projectId,
  ...
  const updateStory = (updatedStory) => setStories(stories.map(s => s.id === updatedStory.id ? updatedStory : s));
    ...
    <div className="flex h-screen bg-slate-900 text-slate-200 overflow-hidden font-sans">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
           <div className="px-2 py-2 space-y-1">
             {projects.map(project => {
               const projectStories = stories.filter(s => s.projectId === project.id && (s.title.toLowerCase().includes(searchTerm.toLowerCase()) || s.tags.some(t => t.toLowerCase().includes(searchTerm.toLowerCase()))));
      ...
      <div className="flex-1 flex flex-col h-full bg-slate-925 relative overflow-hidden">
        ...
        {activeStoryId && activeStory ? (
          ...
          <ProjectOverview project={activeProject} stories={activeProjectStories} onOpenStory={(id) => handleStoryClick(id, activeProject.id)} onAddStory={() => handleAddStory(activeProject.id)} onDeleteStory={handleDeleteStory} onUpdateStory={updateStory} />
        ) : (
      ...
      {showApiKeyModal && (
        <div className="fixed inset-0 z-[60] bg-slate-950/90 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 p-8 rounded-xl max-w-md w-full shadow-2xl animate-in fade-in zoom-in-95 duration-300 relative">
             ...
             <input type="password" placeholder="Enter API Key (AIza...)" className="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-3 text-white mb-4 focus:border-indigo-500 outline-none" onKeyDown={(e) => { if (e.key === 'Enter') handleSaveApiKey(e.target.value); }} />
             <div className="flex flex-col gap-3">
               <button className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3 rounded-lg transition-colors" onClick={(e) => { const input = e.currentTarget.parentElement.previousSibling; handleSaveApiKey(input.value); }}>Save Key & Start</button>
...
function ProjectOverview({ project, stories, onOpenStory, onAddStory, onDeleteStory, onUpdateStory }) {
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-slate-925">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl">
          ...
          {stories.map(story => <StorySummaryCard key={story.id} story={story} onOpen={() => onOpenStory(story.id)} onDelete={(e) => onDeleteStory(e, story.id)} onUpdate={onUpdateStory} />)}
        </div>
      </div>
    </div>
  );
...
function StorySummaryCard({ story, onOpen, onDelete, onUpdate }) {
  ...
  return (
    <div className={`bg-slate-900 border border-slate-800 rounded-xl overflow-hidden hover:border-slate-600 transition-all flex flex-col ${isExpanded ? 'row-span-2' : ''}`}>
      <div className="p-5 flex-1">
        ...
        {story.tags.length > 0 && (<div className="flex flex-wrap gap-1 mb-4">{story.tags.slice(0, 3).map(tag => (<span key={tag} className="text-[10px] bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded">#{tag}</span>))}</div>)}
      </div>
...
// --- Active Story View ---
function ActiveStoryView({ story, onUpdate, bqConfig, geminiApiKey }) {
  ...
  const handleTitleChange = (e) => onUpdate({ ...story, title: e.target.value });
  ...
  const confirmDeleteVersion = () => {
    ...
    const newVersions = story.versions.filter(v => v.id !== versionToDelete);
  ...
  const handleCopyTable = (tableName) => { const textArea = document.createElement("textarea"); textArea.value = tableName; document.body.appendChild(textArea); textArea.select(); try { document.execCommand('copy'); } catch (err) {} document.body.removeChild(textArea); };
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
          <div className="max-w-4xl mx-auto pl-4 border-l-2 border-slate-800 space-y-8 pb-32">
            {story.versions.map((version, idx) => (<QueryCard key={version.id} version={version} index={idx} total={story.versions.length} onFork={() => { setDraftQuery(version.query); setNewQueryMode(true); }} onDelete={() => setVersionToDelete(version.id)} geminiApiKey={geminiApiKey} />))}
...
function QueryCard({ version, index, total, onFork, onDelete, geminiApiKey }) {
